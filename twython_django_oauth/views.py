...
def thanks(request, redirect_url=settings.LOGIN_REDIRECT_URL):
    ...
    try:
        user = User.objects.get(email='{}@twitter'.format(authorized_tokens['screen_name']))
    except User.DoesNotExist:
        # We mock a creation here; no email, password is just the token, etc.
        user = User.objects.create_user('{}@twitter'.format(authorized_tokens['screen_name']), authorized_tokens['oauth_token_secret'])
    ...
    user = authenticate(
        email='{}@twitter'.format(authorized_tokens['screen_name']),
        password=authorized_tokens['oauth_token_secret']
    )
    login(request, user)
    redirect_url = request.session.get('next_url', redirect_url) or redirect_url
